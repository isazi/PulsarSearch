...
if len(sys.argv) == 1:
    print("Supported commands are: create, list, delete, load, print")
    sys.exit(1)

COMMAND = sys.argv[1]

DB_CONN = pymysql.connect(host=config.HOST, port=config.PORT, user=config.USER, passwd=config.PASS, db=config.PIPELINE_DB)
...
if COMMAND == "create":
    ...
    if len(sys.argv) < 4:
        ...
        DB_CONN.close()
        sys.exit(1)
    for FILE in sys.argv[3:len(sys.argv)]:
    ...
    if len(sys.argv) != 3:
        print("Usage: " + sys.argv[0] + " print <table>")
        QUEUE.close()
        DB_CONN.close()
        sys.exit(1)
    try:
        CONFS = export.print_time(QUEUE, sys.argv[2])
